...
show-output-paths:
	...
	print_paths "broad overview HTML entry points for deeper retrospective/review and quick human scanning (history compare, research summary; pair line 1 with line 1 in the matching JSON list):" $(REPORT_OVERVIEW_REVIEW_PATHS); \
	...
	print_paths "matching broad overview JSON companions for deeper retrospective/review automation/downstream tooling, in the same order as the HTML list above for line-for-line visual pairing:" $(REPORT_OVERVIEW_REVIEW_MACHINE_PATHS); \
