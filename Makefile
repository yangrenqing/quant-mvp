...
OVERVIEW_GROUP_PAIRING_NOTE ?= in the grouped broad overview lists below, HTML pages are for quick human scanning, matching JSON companions are for automation or downstream tooling, and the JSON lists stay in the same order as the HTML lists for line-for-line pairing.
