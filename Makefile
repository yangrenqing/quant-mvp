...
help:
	...
	@echo "make show-output-paths # print $(REPORT_OVERVIEW_RECOMMENDED_OUTPUT_PATH) as the single best high-level overview HTML page to open first for quick human scanning with matching machine-readable $(REPORT_OVERVIEW_RECOMMENDED_MACHINE_OUTPUT_PATH) for automation/downstream tooling, make the best overview starting point explicit for during the trading day vs after the close with matching JSON companions, separate the broad overview set into quick operational monitoring pages vs deeper narrative/review pages, and give market-context plus research-wrap-up overview entries the same start-here + matching-JSON treatment, while keeping the overview HTML quick-scan lists and matching JSON automation lists in the same order for easy visual pairing, and keeping the full overview set plus per-run history/archive inspection locations and workflow-specific latest outputs for scan, portfolio, dataset, and model, where scan/portfolio open-this-first latest paths are framed for quick operational monitoring and the immediate post-run check while dataset/model open-this-first latest paths are framed for deeper review and the immediate post-run check, the next line under each latest or archived first-open path shows the closest machine-readable companion, whether each exists on disk, and when latest vs archive recommendations intentionally differ by format"
