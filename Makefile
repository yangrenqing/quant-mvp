...
OVERVIEW_ENTRY_USE_NOTE ?= within the broad overview set, use $(REPORT_DASHBOARD_OVERVIEW_PATH) for live status, $(REPORT_HISTORY_OVERVIEW_PATH) for retrospective analysis, $(REPORT_MARKET_OVERVIEW_PATH) for market context, and $(REPORT_RESEARCH_SUMMARY_PATH) for research wrap-up; use a workflow-specific open-this-first path when you are inspecting one workflow run in detail.
...
show-output-paths:
	...
	print_paths "broad overview entry points by intent (live status -> dashboard, market context -> market overview, retrospective analysis -> history compare, research wrap-up -> research summary):" $(REPORT_OVERVIEW_ENTRY_PATHS); \
	print_paths "broad overview machine-readable companions in the same intent order:" $(REPORT_OVERVIEW_MACHINE_PATHS); \
