...
`make show-output-paths` prints all expected generated report/artifact paths after `make scan`, `make portfolio`, `make dataset`, or `make model`, and it marks each path as currently `present` or `missing` on disk. For `make scan`, the scan section now includes both the main scan report set (`reports/a_share_scan.txt`, `reports/a_share_scan.html`, `reports/a_share_scan.json`) and the focus-only shortlist outputs produced alongside it (`reports/a_share_focus.txt`, `reports/a_share_focus.html`, `reports/a_share_focus.json`). For `make portfolio`, the portfolio section includes the full report set plus companion artifacts: `reports/portfolio_backtest.txt`, `reports/portfolio_backtest.html`, `reports/portfolio_backtest.json`, and `reports/portfolio_backtest.csv`. It also highlights one separate `open this first` default artifact per workflow so operators can distinguish the full artifact set from the best first inspection target: `reports/a_share_scan.html` for `make scan`, `reports/portfolio_backtest.html` for `make portfolio`, `reports/training_dataset.csv` for `make dataset`, and `reports/model_pipeline_latest.txt` for `make model`. For `make model`, the current/latest section under `reports/` includes the primary text summary plus the machine-readable companions already produced by the workflow (`reports/model_predictions.csv`, `reports/benchmark_classifier_predictions.csv`, `reports/linear_model.json`, `reports/benchmark_classifier.json`, and `reports/model_registry.jsonl`), while still keeping those actively consumed outputs distinct from the archived per-run snapshots in `reports/model_versions/`. Use it right after one of those workflow targets finishes when you want the recommended first stop without losing track of the other HTML/JSON/CSV/text outputs generated by that workflow.
