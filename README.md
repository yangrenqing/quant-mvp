...
`make show-output-paths` prints all expected generated report/artifact paths after `make scan`, `make portfolio`, `make dataset`, or `make model`, and it marks each path as currently `present` or `missing` on disk. It now groups each workflow into compact operator buckets: summary views versus structured data/model files, while still highlighting one separate `open this first` default path per workflow so operators can distinguish the best first inspection target from the rest of the output set: `reports/a_share_scan.html` for `make scan`, `reports/portfolio_backtest.html` for `make portfolio`, `reports/training_dataset.csv` for `make dataset`, and `reports/model_pipeline_latest.txt` for `make model`. For `make scan`, the scan section includes both the main scan report set (`reports/a_share_scan.txt`, `reports/a_share_scan.html`, `reports/a_share_scan.json`) and the focus-only shortlist outputs produced alongside it (`reports/a_share_focus.txt`, `reports/a_share_focus.html`, `reports/a_share_focus.json`). For `make portfolio`, the portfolio section labels the text/HTML outputs as summary views and the JSON/CSV companions as structured data/model files. For `make dataset`, it labels the text summary as a summary view and the CSV/JSON exports as structured data/model files. For `make model`, the current/latest section under `reports/` labels the human-readable summaries (`reports/model_pipeline_latest.txt`, `reports/model_train.txt`, and `reports/benchmark_classifier.txt`) as summary views and the workflow's generated CSV/JSON/JSONL outputs (`reports/model_predictions.csv`, `reports/benchmark_classifier_predictions.csv`, `reports/linear_model.json`, `reports/benchmark_classifier.json`, and `reports/model_registry.jsonl`) as structured data/model files, while still keeping those actively consumed outputs distinct from the archived per-run snapshots in `reports/model_versions/`. Use it right after one of those workflow targets finishes when you want the recommended first stop without losing track of the other summary views or structured data/model files generated by that workflow.
