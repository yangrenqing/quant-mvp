...
`make show-output-paths` prints the most useful downstream report/artifact paths to inspect after `make scan`, `make portfolio`, `make dataset`, or `make model`, and it marks each expected path as currently `present` or `missing` on disk. It now groups the main text artifact with its key HTML/JSON/CSV companions, and for `make model` it also separates the current/latest outputs under `reports/` from the versioned history directory at `reports/model_versions/`, so operators can quickly tell which paths are the actively consumed outputs versus the archived per-run snapshots. Use it right after one of those workflow targets finishes when you want to jump from the primary report to its browser-friendly or machine-readable companion without re-checking the workflow docs.
